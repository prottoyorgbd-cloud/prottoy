...
// New vabe eta add korchiiii===== ADMIN RECEIPT GENERATOR =====
document.addEventListener('DOMContentLoaded', () => {
  const form = document.getElementById('adminReceiptForm');
  if (!form) return;

  // Update preview on input change
  form.querySelectorAll('input, select').forEach(el => {
    el.addEventListener('input', updateAdminReceiptPreview);
  });

  // Buttons
  document.getElementById('adminReceiptPdf')?.addEventListener('click', () => generateAdminReceipt('pdf'));
  document.getElementById('adminReceiptPrint')?.addEventListener('click', () => generateAdminReceipt('print'));
  document.getElementById('adminReceiptImage')?.addEventListener('click', () => generateAdminReceipt('image'));

  function updateAdminReceiptPreview() {
    const name = document.getElementById('adminDonorName').value.trim() || '—';
    const amount = Number(document.getElementById('adminAmount').value) || 0;
    const cause = document.getElementById('adminCause').value || '—';
    const signatory = document.getElementById('adminSignatory').value.trim() || 'অনুমোদিত স্বাক্ষর';
    const withQr = document.getElementById('adminWithQr').checked;

    document.getElementById('adminRcName').textContent = name;
    document.getElementById('adminRcAmount').textContent = `৳ ${amount.toLocaleString()}`;
    document.getElementById('adminRcCause').textContent = cause;
    document.getElementById('adminRcSignatory').textContent = signatory;

    // Receipt number & date
    const now = new Date();
    const dateStr = now.toLocaleDateString('bn-BD', { day: 'numeric', month: 'long', year: 'numeric' });
    document.getElementById('adminRcDate').textContent = dateStr;
    const serial = (parseInt(localStorage.getItem('prottoy-receipt-serial') || '0') + 1);
    const ymd = `${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')}`;
    document.getElementById('adminRcNumber').textContent = `PRT-${ymd}-${String(serial).padStart(4,'0')}`;

    // QR Code
    const qrBox = document.getElementById('adminRcQr');
    qrBox.innerHTML = '';
    if (withQr) {
      // QR code লাইব্রেরি লোড করে QR তৈরি করবেন (যদি ইচ্ছে)
    }
  }

  async function generateAdminReceipt(action) {
    const name = document.getElementById('adminDonorName').value.trim();
    const amount = Number(document.getElementById('adminAmount').value);
    const cause = document.getElementById('adminCause').value;
    if (!name || !amount || !cause) {
      alert('দাতার নাম, পরিমাণ ও খাত পূরণ করুন');
      return;
    }

    // প্রিভিউ আপডেট
    updateAdminReceiptPreview();

    // PDF/Print/Image - আগের মতোই কাজ করবে (এখানে সংক্ষিপ্ত)
    if (action === 'print') {
      window.print();
      return;
    }

    // বাকি অপশন (PDF/Image) এর জন্য আগের মতো jsPDF/html2canvas ব্যবহার করুন
    alert(`রসিদ ${action} আকারে তৈরি করা হবে (বাস্তবায়ন করা বাকি)`);
    // সম্পূর্ণ কোড চাইলে জানাবেন
  }

  // প্রথমবার প্রিভিউ সেট করুন
  updateAdminReceiptPreview();
});
